...
export default class Slide {
  ...
  transition (active) {
    this.slide.style.transition = active ? 'transfom .3s' : '';
  }
  ...
  onMove(event) {
    const poinerPosition = (event.type === 'mousemove') ? event.clientX : event.changedTouches[0].clientX; 
    const finalPosition = this.updatePosition(poinerPosition);
